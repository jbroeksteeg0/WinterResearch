...
std::map<std::string, std::set<State>> node_states;
...
void shortest_paths() {
  ...
  node_states["R-D"].insert(initial_state);
  ...
  while (q.size()) {
    ...
    if (node_states[curr_state.node].find(curr_state) == node_states[curr_state.node].end()) {
    ...
    for (const auto &to : node_names) {
      ...
      while (check_iter != node_states[to].end()) {
        if (check_iter->time <= new_state.time) {
          if (check_iter->dominates(new_state)) {
            // std::cout << check_iter->to_string() << " is strictly better than "
            //           << new_state.to_string() << std::endl;
            add_state = false;
            break;
          }
          check_iter++;
          ...
          if (new_state.dominates(*check_iter)) {
            auto prev_iter = check_iter;
            check_iter++;
            node_states[to].erase(prev_iter);    // doesn't invalidate i think
          } else {
            check_iter++;
          }
        }
      ...
      if (add_state) {
        node_states[to].insert(new_state);
        q.push(new_state);
      }
    }
  }
