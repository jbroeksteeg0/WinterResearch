...
void shortest_paths() {
  std::unordered_map<State, double> min_cost;
  ...
  // A priority queue, will be sorted in descending order due to
  // State's < operator being reversed
  ...
  int iterations = 0, states_skipped = 0;
  while (pq.size()) {
    State curr_state = pq.top();
    pq.pop();
    if (iterations % 100000 == 0)
      std::cout << iterations << " iterations, " << states_skipped << " skipped" << std::endl;

    iterations++;

    if (min_cost[curr_state] < curr_state.cost) {
      states_skipped++;
      continue;
    }

    // assert(min_cost.find(curr_state) != min_cost.end());
    // assert(min_cost[curr_state] <= curr_state.cost);

    if (curr_state.node == "R-D") {
      ans = std::min(ans, curr_state.cost);
    }

    std::string from = curr_state.node;
    for (const std::string &to : node_names) {
      // Cannot go from a node to itsself
      if (from == to)
        continue;
      ...
      if (min_cost.find(new_state) == min_cost.end() || min_cost[new_state] > new_state.cost) {
        min_cost[new_state] = new_state.cost;
        // assert(min_cost.find(new_state) != min_cost.end());
        pq.push(new_state);
      }
    }
  }

  std::cout << "Lowest cost is " << ans << std::endl;
}
